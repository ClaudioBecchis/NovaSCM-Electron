...
export default function CertsTab({ addLog }) {
  ...
  useEffect(() => {
    if (showQr && qrCanvasRef.current) {
      const enrollUrl = `${config.certportalUrl || 'http://192.168.20.110:9090'}/enroll?mac=${showQr.mac}&name=${encodeURIComponent(showQr.nome)}`;
        ...
        <Modal title={`QR Code - ${showQr.nome}`} onClose={() => setShowQr(null)}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{
              ...
              {config.certportalUrl || 'http://192.168.20.110:9090'}/enroll?mac={showQr.mac}&name={encodeURIComponent(showQr.nome)}
            </div>
